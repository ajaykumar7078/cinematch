...
@st.cache_resource
def load_all():
    data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'tmdb_5000_movies.csv')
...
if mode == "🎯 Similar Movies":
    ...
    with col2:
        top_n = st.number_input("Count:", 3, 30, 10)
    if search_term:
    ...
    ca.metric("Movies", stats['total_movies'])
    cb.metric("Avg Rating", stats['avg_rating'])
    cc.metric("Years", stats['year_range'])
    cd.metric("Top", stats['top_rated'][:12])
    if results:
        for i, movie in enumerate(results, 1):
            st.markdown(render_card(movie, rank=i), unsafe_allow_html=True)

elif mode == "🎭 Multi-Genre":
    st.markdown('<p class="main-header">🎭 Multi-Genre Search</p>', unsafe_allow_html=True)
    selected_genres = st.multiselect("Genres (must match ALL):", ALL_GENRES, default=["Horror","Thriller"])
    if selected_genres:
        results = explorer.multi_genre_search(selected_genres, top_n=20)
        st.caption(f"Found {len(results)} movies")
        if results:
            for i, movie in enumerate(results, 1):
                st.markdown(render_card(movie, rank=i), unsafe_allow_html=True)
    ...
    for genre in ALL_GENRES:
        s = explorer.genre_stats(genre)
        stats_data.append({"Genre": genre, "Movies": s['total_movies'], "Avg Rating": s['avg_rating'], "Top": s['top_rated']})
